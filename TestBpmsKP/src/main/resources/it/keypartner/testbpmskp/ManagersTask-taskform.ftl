<script type='text/javascript'>
	...
	function taskFormValidator() {
		...
		var myInputs = new Array();
					myInputs[i] = document.getElementById("outp");
					i++;


		var j=0;
						if(notEmpty(myInputs[j]) && !isAlphanumeric(myInputs[j])) {
							alert("Please enter valid outp");
							myInputs[j].focus();
							return false;
						}
...
<div id="container">
	<div id="header">
		User Task Form: DemoBP.ManagersTask
	</div>
	<div id="content">
		...
		<fieldset>
            <legend>Task Inputs</legend>
                            		<label for="name">innome</label>
                            		<div class="div_checkbox">
                              		${innome}
                            		</div>
                            		<label for="name">ineta</label>
                            		<div class="div_checkbox">
                              		${ineta}
                            		</div>
          ...
          <fieldset>
            <legend>Task Outputs</legend>
                            		<label for="name">outp</label>
                            		<div class="div_texbox">
                              		<input name="outp" type="text" class="textbox" id="outp" value="${outp}" />
                            		</div>
