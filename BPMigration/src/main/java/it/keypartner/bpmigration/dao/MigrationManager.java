...
public class MigrationManager {
	...
	//
	// if
	// (!RuntimeManagerRegistry.get().isRegistered(processData.getDeploymentId()))
	// {
	// logger.error("No deployment found for {}",
	// processData.getDeploymentId());
	//
	// return false;
	// }
	//
	// if
	// (!RuntimeManagerRegistry.get().isRegistered(processData.getToDeploymentId()))
	// {
	// logger.error("No deployment found for {}",
	...
	// RuntimeManagerRegistry.get().getManager(
	// processData.getToDeploymentId());
	// if
	// (manager.getEnvironment().getKieBase().getProcess(processData.getToProcessId())
	// == null) {
	// logger.error("No process found for {} in deployment {}",
	// processData.getToProcessId(),
	// processData.getToDeploymentId());
	//
	// return false;
	// }
	//
	// String auditPu =
	// manager.getDeploymentDescriptor().getAuditPersistenceUnit();
	//
	// EntityManagerFactory emf =
	// EntityManagerFactoryManager.get().getOrCreate(auditPu);
	//
	// JPAAuditLogService auditService = new JPAAuditLogService(emf);
	// ProcessInstanceLog log =
	// auditService.findProcessInstance(processData.getProcessInstanceId());
	...
	// processData.getProcessInstanceId(), (log == null ? "-1" :
	// log.getStatus()));
	//
	// return false;
	// }
	...
	// public static Map<String, List<NodeInformation>>
	// collectNodeInfo(ProcessData processData) {
	// Map<String, List<NodeInformation>> result = new HashMap<String,
	...
	//
	// InternalRuntimeManager currentManager = (InternalRuntimeManager)
	// RuntimeManagerRegistry.get().getManager(
	// processData.getDeploymentId());
	// InternalRuntimeManager toBeManager = (InternalRuntimeManager)
	// RuntimeManagerRegistry.get().getManager(
	// processData.getToDeploymentId());
	//
	// org.kie.api.definition.process.Process toBeProcess =
	// toBeManager.getEnvironment().getKieBase()
	// .getProcess(processData.getToProcessId());
	//
	// collectNodeInformation((NodeContainer) toBeProcess, newProcessNodes);
	//
	// String auditPu =
	// currentManager.getDeploymentDescriptor().getAuditPersistenceUnit();
	//
	// EntityManagerFactory emf =
	// EntityManagerFactoryManager.get().getOrCreate(auditPu);
	//
	// JPAAuditLogService auditService = new JPAAuditLogService(emf);
	//
	// List<NodeInstanceLog> logs =
	// auditService.findNodeInstances(processData.getProcessInstanceId());
	...
	public static String migrate(ProcessToMigrate processData) {
		...
		try {
			...
			// convertedNodeMapping.put(map.getSourceNodeId(),
			// map.getTargetNodeId());
			// }
			// }
			upgradeProcessInstance(extractIfNeeded(current), extractIfNeeded(tobe), processData.getProcessInstanceId(),
					processData.getToProcessId(), convertedNodeMapping);
			...
			current.destroy();
			tobe.destroy();
			outcomeBuffer.append("Successful migration of process instance id (" + processData.getProcessInstanceId()
					+ ") to process def [" + processData.getToProcessId() + "] and deployId ["
					+ processData.getToDeploymentId() + "]");
		} catch (Exception e) {
	...
	// private static void collectNodeInformation(NodeContainer container,
	// List<NodeInformation> nodes) {
	...
	// continue;
	// }
	// nodes.add(new NodeInformation(node.getName(), (String)
	// node.getMetaData().get("UniqueId"), String
	// .valueOf(node.getId())));
	// }
	// }
	// }

	// private static void collectActiveNodeInformation(List<NodeInstanceLog>
	// logs, List<NodeInformation> activeNodes) {
	// Map<String, NodeInstanceLog> nodeInstances = new HashMap<String,
	// NodeInstanceLog>();
